...
(function (global) {
  ...
  //  data-i18n-html="dot.path"       -> innerHTML
  //  data-i18n-attr="attr:dot.path"  -> setAttribute(attr, value)
  //
  // The HTML variant exists because the design carries markup inside single
  // strings: the hero headline's <em> highlighter band, the <br> breaks that
  // set the line rhythm of display headings, the <b> lead-in on the homepage
  // lede. Through textContent those arrive as literal angle brackets and the
  // band disappears, so those strings have to be written as HTML and the
  // translations have to carry the same tags.
  //
  // The values come from our own locale files, served from this origin and
  // written by us - the same trust level as the generated markup itself.
  // Never point this binding at anything a visitor can supply.
